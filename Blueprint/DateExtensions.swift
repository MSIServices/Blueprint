...
extension Date {
    ...
    //Swift Dates
    func formatDateFrom(outputFormat: String) -> String {
...
extension NSDate {
    ...
    func formatDateFrom(outputFormat: String) -> String {
        ...
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = outputFormat
        let formattedDate = dateFormatter.string(from: self as Date)
    ...
    /// Returns the amount of weeks from another date
    func weeks(from date: Date) -> Int {
        return Calendar.current.dateComponents([.weekOfMonth], from: date, to: self as Date).weekOfMonth ?? 0
    }
    /// Returns the amount of days from another date
    func days(from date: Date) -> Int {
